package com.crypto.cryptohash.core
...
fun beforeLeft(vararg items: EitherBlock<Exception, *>): Exception? {
...
fun <R> beforeRight(vararg items: EitherBlock<Exception, R>): R? {
    var result: R? = null
    for (item in items) {
...
fun <T> runBeforeLeft(vararg items: () -> Either<Exception, T>): Either<Exception, Boolean> {
    return runBeforeLeft(items.toList())
}

fun <T> runBeforeLeft(items: List<() -> Either<Exception, T>>): Either<Exception, Boolean> {
    var result: Either<Exception, T>? = null
    for (item in items) {
        result = item()
        if (result.isLeft) {
            break
...
suspend fun ifEitherSuspend(value: Boolean, block: suspend () -> Either<Exception, Boolean>): Either<Exception, Boolean> {
    return if (value) {
        block()
    } else {
        true.toRight()
...
fun ifEither(value: Boolean, block: () -> Either<Exception, Boolean>): Either<Exception, Boolean> {
    return if (value) {
        block()
    } else {
        true.toRight()
